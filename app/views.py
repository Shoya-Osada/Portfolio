...
#プロフィールのデータを降順で並び変えて、テンプレートに渡す。
class IndexView(View):
    def get(self, request, *args, **kwargs):
        profile_data = Profile.objects.all()
        if profile_data.exists():
            profile_data = profile_data.order_by("-id")[0]
        #新しい順に並べたいので、order_byを使用して降順に並べ替え
...
#View クラスを継承し、get 関数を post 関数を用意します。
class ContactView(View):
    #お問い合わせフォームの中身は何もなくていいので、空のフォームをテンプレートに渡します。
    def get(self, request, *args, **kwargs):
    ...
    def post(self, request, *args, **kwargs):
        ...
        #フォームの内容が正しいかform.is_valid()でバリデーションをしてから、form.cleaned_dataで名前やメールアドレス、メッセージを取得します。
        if form.is_valid():
            name = form.cleaned_data['name']
            email = form.cleaned_data['email']
            message = form.cleaned_data['message']
            subject = 'お問い合わせありがとうございます。'
            #textwrap.deden関数を使用すると、共通して現れる先頭の空白を削除してくれます。
            ...
            #EmailMessage関数を使用することで、多くの設定をすることができます。
            #シンプルな設定でいい場合は、send_mail関数を使用して、メール送信します。
            try:
